...
export const GameField = (props) => {
    ...
    const item = localGameItems.map(i => <NavLink to='/play' style={props.theme.buttonsColors} className={i}
                                                  onClick={() => onItemSelect(i, localGameItems.length)}
                                                  key={i}></NavLink>)
    return (
        <div className={styles.field}>
            <img src={triangle} alt="triangle" className={styles.field__img}/>
            {item}
            <button style={props.theme.buttonsColors} className={styles.field__btn} onClick={onSpockMode}>Switch Spock
                Mode
            </button>
        </div>
    )
